...
public class ImportacaoService {
    ...
    private void parseHeader(String linha, Curso curso) {
        curso.setNomeCurso(linha.substring(1, 51).trim());
        ...
        curso.setSequenciaArquivo(Integer.parseInt(linha.substring(73, 80).trim()));
        curso.setVersaoLayout(linha.substring(80, 83).trim());
    }

    private Fase parseFase(String linha) {
        Fase fase = new Fase();
        fase.setNomeFase(linha.substring(1, 8).trim());
        return fase;
    }
    ...
    private Disciplina parseDisciplina(String linha) {
        ...
        disciplina.setDiaSemana(Integer.parseInt(linha.substring(6, 8).trim()));
        return disciplina;
    }

    private Professor parseProfessor(String linha) {
        Professor professor = new Professor();
        professor.setNomeProfessor(linha.substring(1, 41).trim());
        professor.setTituloDocente(Integer.parseInt(linha.substring(41, 43).trim()));
        return professor;
    }
}